...
print("RMSD(Model 2)=$ErrC")

# Model 3: all time fractional
...
print("RMSD(Model 3)=$ErrF")

# Model 4 using 2 sections: first integer until 250 then fractional
...
α=[0.5000000002428997,0.6734861099275703,0.5563512458535832,0.6759832954065609 ,0.5000000043012366,0.5502587715278799,0.8839529214763222,0.5000000000260537]
...
scatter(data[:,1],data[:,2], label="Real data", c="khaki3",markerstrokewidth=0)
	plot!(t,AppxC,ylabel=L"\textrm{Cumulative\;\;cases} \; (\mathrm{10^3})",lw=2, label="Model 1, RMSD=2085", c="darkorange3", linestyle=:dashdot)
	plot!(t,Appx1,lw=2, label="Model 2, RMSD=276.5", c="deepskyblue1",guidefont=font(12,plot_font),
	yticks = ([0,5000,10000,15000],string.([0,5,10,15])),fontfamily= plot_font)
	plot!(t,AppxF,lw=2, label="Model 3, RMSD=245.9", c="deeppink", linestyle=:dash)
	plotModels=plot!(t,AppxF2,xlabel="Days", legendposition=:bottomright, lw=2, label="Model 4, RMSD=170.9", linestyle=:dot, c="blue1",
	title = "(a)", titleloc = :left, titlefont = font(10))

#plot population
plot(t,Nc*ones(length(t)),c="darkorange3",lw=2,legendposition=:right,label="Model 1", linestyle=:dashdot)
	plot!(t,N,lw=2,legendposition=:right,label="Model 2", c="deepskyblue1")
	plot!(t,Nf,lw=2,legendposition=:right,label="Model 3",linestyle=:dash,c="deeppink")
	plotN=plot!(t,Nf2,label="Model 4",linestyle=:dot, lw=2, xlabel="Days",guidefont=font(12,plot_font),
...
##plot optimized initial time for FDE

input_file = "Output_Err_time_216_300.txt"
ErrvsTime = readdlm(input_file)
# EvsT= sort(ErrvsTime[:,2])
EvsT=ErrvsTime[sortperm(ErrvsTime[:, 2]), :]
EvsT2=LinearInterpolation(EvsT[:,2] .+ 215, EvsT[:,1])
InitialFDE=scatter(216:300,EvsT2(216:300),
		xlabel="Initial time for FDE in Model 4",
		ylabel="RMSD",c="black",markerstrokewidth=0,label=:false,
		title = "(c) ", titleloc = :left, titlefont = font(10))
# min_val, min_idx = findmin(EvsT[:,2]) # 64 is the min -> 64+216=280
...
for i=1:8
    ...
    for j=1:40
        # print(j)# to check in which itteration is counting
        α=ones(8)
        t1, x1= FDEsolver(F, [4,280], x0, α, par, nc=2,h=.01)
        x02=x1[end,:]
        α[i]=1-j*.01
        t2, x2= FDEsolver(F, [280,438], x02, α, par, nc=2,h=.01)
        x=vcat(x1,x2[2:end,:])
        t=vcat(t1,t2[2:end,:])
        S1=x[:,1]; E1=x[:,2]; I1=x[:,3]; R1=x[:,4];
        L1=x[:,5]; H1=x[:,6]; B1=x[:,7]; C1=x[:,8];
...
heatmap(["S","E","I","R","L","H","B","C"],0.6:.01:.99,log10.(Errr[:,end:-1:1]'),
		...
		title = "(f) Model 4", titleloc = :left, titlefont = font(10))
		pltheat=plot!(Shape(0 .+ [1,2,2,1], 0 .+ [.595,.595,.995,.995]), fillcolor=:false, linecolor=:red,linestyle=:dot, legend=:false, w=3)


# plot heatmap model 3
pltheat1=heatmap(["S","E","I","R","L","H","B","C"],0.6:.01:.99,log10.(Err[:,end:-1:1]'),
        color=CoLor,colorbar_title="log10(RMSD)",clim=(RMSD1-.18,RMSD1+.21),
        xlabel="Individual classes", ylabel="Order of derivative",fontfamily= plot_font,
		title = "(e) Model 3 ", titleloc = :left, titlefont = font(10), colorbar=:false)
...
for i=1:Nn
            ...
            plot!(ttt[28000:100:end],Appx[28000:100:end,:],lw=2; color="blue1")

end

scatter!(data[85:end,1],data[85:end,2], label="Real data", c="khaki3",markerstrokewidth=0)
pltFtry=plot!(tt[27000:50:end],Appx11[27000:50:end,:],xlabel="Days",
        lw=2, c="deepskyblue1",label="Integer model",legend=:false,
		ylabel=L"\textrm{Cumulative\;\;cases} \; (\mathrm{10^3})",fontfamily= plot_font,
		yticks = (1000 .*[13,14,15],string.([13,14,15])),
		title = "(d)", titleloc = :left, titlefont = font(10))

lab = @layout [a{0.7w, .4h} b; _ _ ; _ _]
lcd = @layout [_ _ ;a{.25h} b; _ _]
lef = @layout [_ _ ; _ _ ;a{.25h, .4w} b]

plot_ab=plot(plotModels, plotN, layout= lab, size = (900, 900))
plot_cd=plot(InitialFDE, pltFtry, layout= lcd, size = (900, 900))
plot_ef=plot(pltheat1, pltheat, layout= lef, size = (900, 900))

savefig(plot_ab,"plot_ab.svg")
savefig(plot_cd,"plot_cd.svg")
savefig(plot_ef,"plot_ef.svg")
